...
export default class CustomTable extends Component {
  ...
  render() {
    ...
    const renderHitRate = (value, index, record) => {
      return (
        <div>
          {record.hitRate}%
        </div>
    ...
    const renderProductIcon = (value, index, record) => {
      return (
        <Img
          width={100}
          height={100}
          src={record.productIcon}
          type="cover"
        />
      );
    };

    const renderWinBgImgUrl = (value, index, record) => {
      return (
        <Img
          width={100}
          height={110}
          src={record.winBgImgUrl}
          type="cover"
        />
      );
    };
    ...
    return (
      <div style={style}>
        ...
        >
          ...
          <Table.Column title="抽奖活动" dataIndex="prizeTitle" />
          <Table.Column title="奖品名称" dataIndex="productName" />
          <Table.Column title="奖品库存" dataIndex="productTotal" />
          <Table.Column title="奖品图片" dataIndex="productIcon" cell={renderProductIcon} />
          <Table.Column title="中奖背景" dataIndex="winBgImgUrl" cell={renderWinBgImgUrl} />
          <Table.Column title="中奖概率" dataIndex="hitRate" cell={renderHitRate} />
